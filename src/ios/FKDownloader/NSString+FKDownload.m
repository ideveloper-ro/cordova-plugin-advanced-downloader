...
- (NSString *)encodeEscapedString {
    ...
    if (range.location != NSNotFound) {
        ...
        NSString *encode = [[[query componentsSeparatedByString:@"&"] map:^id(NSString *obj, NSUInteger idx) {
            ...
            return [NSString stringWithFormat:@"%@=%@", _AFPercentEscapedStringFromString(key), _AFPercentEscapedStringFromString(value)];
...
/** AFNetworking 借用
 ...
 - returns: The percent-escaped string.
 */
NSString * _AFPercentEscapedStringFromString(NSString *string) {
    static NSString * const kAFCharactersGeneralDelimitersToEncode = @":#[]@"; // does not include "?" or "/" due to RFC 3986 - Section 3.4
    static NSString * const kAFCharactersSubDelimitersToEncode = @"!$&'()*+,;=";
